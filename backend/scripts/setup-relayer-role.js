 ...
 * 
 * Executar: node scripts/setup-relayer-role.js
 */
...
async function main() {
  ...
  if (!process.env.RELAYER_PRIVATE_KEY) {
    console.error('❌ RELAYER_PRIVATE_KEY não configurada no .env');
    process.exit(1);
  }

  if (!contracts.deathNotification) {
    console.error('❌ DEATH_NOTIFICATION_ADDRESS não configurado no .env');
    process.exit(1);
  }
